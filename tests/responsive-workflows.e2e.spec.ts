...
test('phone login keeps both fields and the submit action visible', async ({ page }) => {
  ...
  await page.goto('/?e2e=V2026.08.15.15', { waitUntil: 'domcontentloaded' });
...
test('Home is two square columns on phones and six complete columns on desktop', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  expect(phone).toEqual({ count: 12, columns: 2, square: true, overflow: 0 });
