...
export async function installHlOrderFixture(page, baseURL, options = {}) {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (url.pathname.endsWith('/functions/v1/app-api')) {
      ...
      if (body.action === 'db' && String(body.method).toUpperCase() === 'GET') return json(route, { ok: true, data: body.table === 'ph_soc_master' ? control.rows : body.table === 'ph_master_inventory' ? inventoryReadFixture.read(control.master, body.query || '').rows : [] });
