...
import { installHlOrderFixture, hlMaster, hlUserId } from './hl-order-state.mjs';
...
const source = (number: number, name: string) => ({
  id: id(number), revision: 1, customer_key: customerKey, claims: [], canAuthorizeRepeat: false,
...
export async function installSalesMobileFixture(page: Page, baseURL: string) {
  ...
  const history = Array.from({ length: 63 }, (_, index) => {
    ...
    return { id: id(100 + index), snapshot, ...snapshot, status: 'completed', sort_at: at, customerKey };
  });
  const pendingAt = '2026-09-20T14:00:00Z';
  history.push({ ...history[0], id: id(190), commonname: 'Pending Magnolia', status: 'pending', sort_at: pendingAt,
    snapshot: { ...history[0].snapshot, commonname: 'Pending Magnolia', date_completed: '', created_at: pendingAt, req_status: 'PENDING' } });
  ...
  const control = {
    sources, history, drafts, submissions, attachments, commands, navigation, productionRows, inventoryRows,
    failUploads: false, loseSubmitAcknowledgement: false, rejectShortcutSave: false,
    denyHistory: false, contractErrors: [] as string[], native: null as any,
    seedReview() {
  ...
  const contract = (body: any) => {
    ...
    if (body.action === 'request_history') {
      ...
      if (op === 'folders') return { folders: [{ ...folder, count: history.length }], nextCursor: null };
      if (op === 'search') {
        const query = String(p.query || '').trim().toLowerCase();
        return pageRows(history.filter(row => (p.status === 'all' || row.status === (p.status || 'completed')) &&
          (!p.customerKey || row.customerKey === p.customerKey) && row.snapshot.commonname.toLowerCase().includes(query))
          .sort((a, b) => b.sort_at.localeCompare(a.sort_at) || b.id.localeCompare(a.id)), p);
      }
      fail(`Unexpected history operation ${op}`, 400);
    }
    if (op === 'folders') return { folders: [copy(folder)], nextCursor: null };
    if (op === 'sources') return { ...pageRows(sources.filter(row => row.customer_key === p.customerKey &&
    ...
    if (op === 'attachment_upload') {
      ...
      if (!sources.some(row => row.id === p.sourceId) || !p.mime?.startsWith('image/') || !p.base64) fail('INVALID_ATTACHMENT', 400);
    ...
    if (op === 'save_draft') {
      ...
      const draft = { ...copy(p), customer_key: p.customerKey, snapshot: copy(sources[0].snapshot),
        lines: p.lines.map((line: any) => ({ ...copy(line), snapshot: copy(sources.find(row => row.id === line.sourceId)?.snapshot) })),
  ...
  control.native = await installHlOrderFixture(page, baseURL, {
    ...
    runtimeRequest: async (route: Route) => {
      // Registered before compiled runtime executes so initial compatibility and
      // permission reads use the same contracts as subsequent navigation.
      if (!installed) { installed = true; await page.route('**/functions/v1/app-api', handle); }
