...
async function fixture(page:Page,baseURL:string,username='dylan_collyge') {
  ...
  // The preview is a top-layer dialog; dismiss unrelated notification UI through its real control event.
  ...
  await page.addLocatorHandler(page.locator('#mobile-push-enable-prompt'), async prompt => {
    await prompt.getByRole('button',{name:'Dismiss',exact:true}).dispatchEvent('click');
  });
