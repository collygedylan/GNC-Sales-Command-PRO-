(function (global) {
  ...
  class Builder {
    ...
    async openCamera() { if (this.cameraMode !== 'video') { this.camera.stop(); this.ui.videoEmpty.style.display = 'flex'; this.setCameraStatus(this.cameraMode === 'portrait' ? 'Opening device camera. Use your phone portrait option if available.' : 'Opening device camera for the selected flyer slot.'); return await this.pickFile(this.selectedSlot, { camera: true }); } await this.camera.open(this.ui.video, 'video'); this.ui.videoEmpty.style.display = 'none'; this.setCameraStatus(`Video camera live. Selected slot: ${this.selectedSlot + 1}.`); }
    async captureSelected() { if (this.cameraMode !== 'video') return await this.openCamera(); if (!this.camera.stream) await this.openCamera(); const shot = await this.camera.capturePhoto(); const card = this.state.cards[this.selectedSlot]; card.imageSrc = shot.dataUrl; card.imageName = `${safeName(this.state.title)}-${this.selectedSlot + 1}.jpg`; this.renderUi(); this.render(); this.setCameraStatus(`Captured photo into slot ${this.selectedSlot + 1}.`); }
    async pickFile(index, options) { const opts = Object.assign({ camera: true }, options || {}); const input = document.createElement('input'); input.type = 'file'; input.accept = 'image/*'; if (opts.camera) input.capture = 'environment'; const file = await new Promise((resolve) => { input.addEventListener('change', () => resolve((input.files && input.files[0]) || null), { once: true }); input.click(); }); if (!file) return null; this.state.cards[index].imageSrc = await blobToDataUrl(file); this.state.cards[index].imageName = file.name; this.renderUi(); this.render(); this.setCameraStatus(this.cameraMode === 'portrait' ? `Portrait photo added to slot ${index + 1}.` : `Photo added to slot ${index + 1}.`); return file; }
