...
-- Extend only the three explicitly approved users. Browser actor IDs are never trusted.
create function private.photo_history_require_actor_v1(p_actor_id uuid) returns text
language plpgsql security definer set search_path='' as $$
declare actor_username text;
begin
  if not private.is_service_role_request() then
    raise exception using errcode='42501',message='PHOTO_HISTORY_FORBIDDEN';
  end if;
  select lower(btrim(p.username)) into actor_username
  from public.profiles p
  where p.id=p_actor_id
    and lower(btrim(p.username)) in ('dylan_collyge','madison_austin','madelyn_gray')
...
revoke all on function private.photo_history_require_actor_v1(uuid) from public,anon,authenticated;
grant execute on function private.photo_history_require_actor_v1(uuid) to service_role;
...
declare
  ...
  required_copies jsonb; recipient_emails jsonb; actor_username text;
  q text; page_size integer; cursor_at timestamptz; cursor_id uuid; as_of timestamptz;
begin
  actor_username := private.photo_history_require_actor_v1(p_actor_id);
  if p_operation='search' then
    ...
    values('photo-history:'||share_id,'photo_history_share',jsonb_build_object('contractVersion','photo-history-share-v2',
      'shareId',share_id,'actorUsername',actor_username,'recipientEmails',recipient_emails,
