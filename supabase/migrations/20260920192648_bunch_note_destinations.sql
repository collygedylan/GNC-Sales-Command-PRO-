begin;
-- The imported sales-year column is named saleyear; snapshots expose salesyear.
-- Sales year is independent of Season and lot naming; unknown never matches.
create function bunch_note_private.sales_year(v text) returns text language sql immutable set search_path='' as $$
 select case when btrim(v) ~ '^[0-9]{2}$' then '20'||btrim(v)
 when btrim(v) ~ '^[0-9]{4}$' then btrim(v) else null end
$$;
create function bunch_note_private.validate_destination(a jsonb, source jsonb) returns void
language plpgsql stable set search_path='' as $$
declare targets jsonb; item text; yr text; dest text:=upper(btrim(coalesce(a->>'destination','')));
begin
 if a->>'destination_mode' is not null and a->>'destination_mode' not in ('matching','other','typed') then raise exception 'BUNCH_NOTE_DESTINATION_INVALID'; end if;
 if length(dest)>200 then raise exception 'BUNCH_NOTE_DESTINATION_INVALID'; end if;
 if a->>'destination_mode'='matching' then
  select jsonb_agg(r),min(upper(btrim(r->>'itemcode'))),min(bunch_note_private.sales_year(r->>'salesyear')) into targets,item,yr
  from jsonb_array_elements(source) r where a->>'scope'='location' or a->'row_ids' ? (r->>'unique_id');
  if yr is null or item is null or item='' or targets is null or exists(select 1 from jsonb_array_elements(targets) r
    where upper(btrim(r->>'itemcode')) is distinct from item or bunch_note_private.sales_year(r->>'salesyear') is distinct from yr)
    then raise exception 'BUNCH_NOTE_SOURCE_YEAR_REQUIRED'; end if;
  if not exists(select 1 from public.ph_master_inventory m where upper(btrim(m.locationcode))=dest
    and upper(btrim(m.itemcode))=item and bunch_note_private.sales_year(m.saleyear)=yr)
    then raise exception 'BUNCH_NOTE_DESTINATION_CHANGED'; end if;
 end if;
end $$;

-- Actor has already been verified by the authenticated command boundary.
create function bunch_note_private.destination_lookup(actor public.profiles,p jsonb) returns jsonb
language plpgsql stable set search_path='' as $$
declare j bunch_note_private.jobs; source jsonb; targets jsonb; item text; yr text; matching jsonb; locations jsonb;
begin
 if jsonb_typeof(p->'source_ids') is distinct from 'array' or jsonb_array_length(p->'source_ids') not between 1 and 500
 then raise exception 'BUNCH_NOTE_ACTION_ROWS_INVALID'; end if;
 if nullif(p->>'job_id','') is not null then
  select * into j from bunch_note_private.jobs where id=(p->>'job_id')::uuid;
  if not found or not bunch_note_private.can_read(actor,j) then raise exception 'BUNCH_NOTE_NOT_FOUND' using errcode='42501'; end if;
  source:=bunch_note_private.work_source(j);
 else
  if actor.username<>'dylan_collyge' then raise exception 'BUNCH_NOTE_AUTHOR_ONLY' using errcode='42501'; end if;
  select coalesce(jsonb_agg(jsonb_build_object('unique_id',m.unique_id,'itemcode',m.itemcode,'salesyear',m.saleyear)),'[]') into source
  from public.ph_master_inventory m where p->'source_ids' ? m.unique_id;
 end if;
 if exists(select 1 from jsonb_array_elements_text(p->'source_ids') id where not exists(select 1 from jsonb_array_elements(source) r where r->>'unique_id'=id))
 then raise exception 'BUNCH_NOTE_ACTION_ROWS_INVALID'; end if;
 select jsonb_agg(r),min(upper(btrim(r->>'itemcode'))),min(bunch_note_private.sales_year(r->>'salesyear')) into targets,item,yr
 from jsonb_array_elements(source) r where p->'source_ids' ? (r->>'unique_id');
 if exists(select 1 from jsonb_array_elements(targets) r where upper(btrim(r->>'itemcode')) is distinct from item or bunch_note_private.sales_year(r->>'salesyear') is distinct from yr)
 then raise exception 'BUNCH_NOTE_SOURCE_YEAR_REQUIRED'; end if;
 select coalesce(jsonb_agg(jsonb_build_object('unique_id',m.unique_id,'locationcode',upper(btrim(m.locationcode)),
  'itemcode',m.itemcode,'salesyear',m.saleyear,'contsize',m.contsize,'lotcode',m.lotcode,'stock',m.ptronhand,'review',m.ptrreviewed,'available',m.ptravailable)
  order by m.locationcode,m.unique_id),'[]') into matching from public.ph_master_inventory m
 where upper(btrim(m.itemcode))=item and bunch_note_private.sales_year(m.saleyear)=yr and nullif(btrim(m.locationcode),'') is not null;
 select coalesce(jsonb_agg(loc order by loc),'[]') into locations from (select distinct upper(btrim(m.locationcode)) loc
 from public.ph_master_inventory m where nullif(btrim(m.locationcode),'') is not null) q;
 return jsonb_build_object('itemcode',item,'salesyear',yr,'matching',matching,'locations',locations);
end $$;

create function bunch_note_private.destination_view(actor public.profiles,p_location text default null) returns jsonb
language plpgsql stable set search_path='' as $$
declare j bunch_note_private.jobs; a jsonb; x jsonb; body jsonb; incoming jsonb:='[]'; own_jobs jsonb:='[]'; locations text[];
 dest text:=nullif(upper(btrim(p_location)),''); planned_dest text; actual_rows jsonb; targets jsonb;
begin
 select array_agg(distinct upper(btrim(locationcode))) into locations from public.ph_master_inventory where nullif(btrim(locationcode),'') is not null;
 -- Filter access before collecting locations, counts or instruction contents.
 for j in select * from bunch_note_private.jobs job where bunch_note_private.can_read(actor,job) loop
  locations:=array_append(locations,upper(btrim(j.location)));
  if upper(btrim(j.location))=dest then own_jobs:=own_jobs||jsonb_build_array(bunch_note_private.job_json(j)); end if;
  body:=bunch_note_private.job_json(j);
  for a in select value from jsonb_array_elements(bunch_note_private.actions(j)) loop
   if bunch_note_private.action_kind(a) not in ('move','hauling') then continue; end if;
   planned_dest:=nullif(upper(btrim(a->>'destination')),'');
   if planned_dest is not null then locations:=array_append(locations,planned_dest); end if;
   select coalesce(jsonb_agg(v),'[]') into actual_rows from jsonb_array_elements(body->'actuals') v
    where v->>'action_id'=a->>'id' and not (v->>'superseded')::boolean and upper(btrim(v->>'destination'))=dest;
   if dest is not null and (planned_dest=dest or jsonb_array_length(actual_rows)>0) then
    select coalesce(jsonb_agg(r),'[]') into targets from jsonb_array_elements(body->'body'->'source') r where a->>'scope'='location' or a->'row_ids' ? (r->>'unique_id');
    incoming:=incoming||jsonb_build_array(jsonb_build_object('job_id',j.id,'note_number',j.note_number,'source_location',j.location,
     'status',j.status,'action',a,'source',targets,'planned_here',planned_dest=dest,'actuals',actual_rows));
   end if;
  end loop;
  for x in select value from jsonb_array_elements(body->'actuals') where not (value->>'superseded')::boolean loop
   if nullif(btrim(x->>'destination'),'') is not null then locations:=array_append(locations,upper(btrim(x->>'destination'))); end if;
   -- A revised instruction can remove an action; its actual work still belongs in the destination history.
   if dest is not null and upper(btrim(x->>'destination'))=dest and not exists(select 1 from jsonb_array_elements(bunch_note_private.actions(j)) entry where entry->>'id'=x->>'action_id') then
    incoming:=incoming||jsonb_build_array(jsonb_build_object('job_id',j.id,'note_number',j.note_number,'source_location',j.location,
      'status',j.status,'action',x->'action_snapshot','source',jsonb_build_array(x->'source_snapshot'),'planned_here',false,'actuals',jsonb_build_array(x)));
   end if;
  end loop;
 end loop;
 return jsonb_build_object('locations',(select coalesce(jsonb_agg(v order by v),'[]') from (select distinct unnest(locations) v) q),
  'location',dest,'jobs',own_jobs,'incoming',incoming);
end $$;
...
language sql stable security definer set search_path='' as $$
 select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'salesyear',m.saleyear,'season',to_jsonb(m)->>'season','desigitem',to_jsonb(m)->>'desigitem',
...
insert into bunch_note_private.options(category,label,kind) values('grading','Grade and Save / Move To','move') on conflict do nothing;
...
revoke all on all functions in schema bunch_note_private from public,anon,authenticated;
