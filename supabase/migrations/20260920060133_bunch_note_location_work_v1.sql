...
revoke all on schema bunch_note_private from public, anon, authenticated;
create sequence bunch_note_private.note_number;
create table bunch_note_private.batches (
 id uuid primary key, revision bigint not null default 1, created_by uuid not null,
 block text not null, body jsonb not null, updated_at timestamptz not null default now()
);
create table bunch_note_private.jobs (
 id uuid primary key, batch_id uuid not null references bunch_note_private.batches(id),
 note_number text not null unique, block text not null, location text not null,
 status text not null default 'open' check(status in ('open','complete','cancelled')),
 owner_id uuid, revision bigint not null default 1, instruction_revision integer not null default 1,
 body jsonb not null, progress jsonb not null default '{}', created_by uuid not null,
 updated_at timestamptz not null default now()
);
create unique index bunch_note_one_open_location on bunch_note_private.jobs(location) where status='open';
create index bunch_note_owner_status on bunch_note_private.jobs(owner_id,status);
create table bunch_note_private.previews (
 id uuid primary key, batch_id uuid not null references bunch_note_private.batches(id),
 created_by uuid not null, batch_revision bigint not null, reports jsonb not null,
 recipients jsonb not null, created_at timestamptz not null default now(),
 expires_at timestamptz not null default now()+interval '30 minutes',
 pdfs jsonb, published boolean not null default false, event_id uuid unique,
 delivery_status text not null default 'not_sent'
);
create table bunch_note_private.versions (
 job_id uuid not null references bunch_note_private.jobs(id), instruction_revision integer not null,
 preview_id uuid not null references bunch_note_private.previews(id), report jsonb not null,
 primary key(job_id,instruction_revision)
);
create table bunch_note_private.audit (
 id bigint generated always as identity primary key, job_id uuid,
 actor_id uuid not null, operation text not null, detail jsonb not null,
 created_at timestamptz not null default now()
);
create table bunch_note_private.commands (
 id uuid primary key, actor_id uuid not null, request jsonb not null, response jsonb not null
);
do $$ declare t text; begin
 foreach t in array array['batches','jobs','previews','versions','audit','commands'] loop
  execute format('alter table bunch_note_private.%I enable row level security',t);
 end loop;
end $$;

create function bunch_note_private.actor(p_id uuid) returns public.profiles
...
begin
 ...
 select * into a from public.profiles where id=p_id and disabled_at is null
  and (locked_until is null or locked_until<=now()) and must_change_password is false;
 if not found then raise exception 'BUNCH_NOTE_ACTOR_INACTIVE' using errcode='42501'; end if;
 return a;
end $$;

-- All creator inventory reads use the complete Park Hill source, never Drive UI filters.
-- These fields are snapshots only; no function in this schema writes inventory.
create function bunch_note_private.inventory(p_block text,p_location text default null) returns jsonb
language sql stable security definer set search_path='' as $$
 select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'season',coalesce(to_jsonb(m)->>'s_lts',to_jsonb(m)->>'season',''),
  'stock',m.ptronhand,'available',m.ptravailable,
...
create function bunch_note_private.recipients(p_ids jsonb) returns jsonb
language plpgsql stable security definer set search_path='' as $$
declare d uuid; result jsonb; n integer;
begin
 if jsonb_typeof(p_ids) is distinct from 'array' or jsonb_array_length(p_ids)>50 then raise exception 'BUNCH_NOTE_RECIPIENT_INVALID'; end if;
 select id into d from public.profiles where username='dylan_collyge' and disabled_at is null
  and must_change_password is false and (locked_until is null or locked_until<=now());
 if d is null then raise exception 'BUNCH_NOTE_DYLAN_EMAIL_REQUIRED'; end if;
 select count(distinct v) into n from jsonb_array_elements_text(p_ids||jsonb_build_array(d)) v;
 select jsonb_agg(x order by x->>'email') into result from (
  select jsonb_build_object('profile_id',p.id,'username',p.username,'display',p.display_name,
   'email',lower(btrim(u.email))) x
  from public.profiles p join auth.users u on u.id=p.id
  where p.id in(select v::uuid from jsonb_array_elements_text(p_ids||jsonb_build_array(d)) v)
   and p.disabled_at is null and p.must_change_password is false and (p.locked_until is null or p.locked_until<=now())
   and u.email ~ '^[^[:space:]@]+@[^[:space:]@]+[.][^[:space:]@]+$'
 ) q;
 if not exists(select 1 from jsonb_array_elements(coalesce(result,'[]')) r where r->>'profile_id'=d::text)
 then raise exception 'BUNCH_NOTE_DYLAN_EMAIL_REQUIRED'; end if;
 if jsonb_array_length(result)<>n then raise exception 'BUNCH_NOTE_RECIPIENT_INVALID'; end if;
 select jsonb_agg(x order by x->>'email') into result from (
  select distinct on(r->>'email') r as x from jsonb_array_elements(result) r
  order by r->>'email',(r->>'username'='dylan_collyge') desc,r->>'profile_id'
 ) deduplicated;
 return result;
end $$;

create function bunch_note_private.can_read(p_actor public.profiles,p_job bunch_note_private.jobs) returns boolean
...
create function public.bunch_note_command_v1(p_actor_id uuid,p_operation text,p_payload jsonb default '{}',
...
begin
 ...
 if p_operation in ('blocks','inventory','directory','drafts','save','preview','publish','revise','assign','cancel','retry','reconcile') and not author
 then raise exception 'BUNCH_NOTE_AUTHOR_ONLY' using errcode='42501'; end if;
 if p_operation='blocks' then
  ...
  return jsonb_build_object('jobs',(select coalesce(jsonb_agg(to_jsonb(j)||jsonb_build_object('delivery_status',
  ...
  return jsonb_build_object('job',to_jsonb(job),'versions',(select coalesce(jsonb_agg(jsonb_build_object(
 ...
 if p_operation='save' then
  ...
  for entry in select value from jsonb_array_elements(body->'locations') loop
   ...
   snapshot:=(select coalesce(jsonb_agg(r order by r->>'unique_id'),'[]') from jsonb_array_elements(source) r where entry->'row_ids' ? (r->>'unique_id'));
   reports:=reports||jsonb_build_array(entry||jsonb_build_object('location',location_value,'source',snapshot,'source_all',source));
  end loop;
  body:=jsonb_build_object('block',upper(btrim(body->>'block')),'locations',reports,'recipient_ids',coalesce(body->'recipient_ids','[]'));
  ...
  if coalesce((p_payload->>'send_email')::boolean,false) then
   insert into public.ph_request_delivery_outbox(event_key,event_type,payload,delivery_mode)
    values('bunch-note:'||preview.id,'bunch_note_submission',jsonb_build_object('preview_id',preview.id),'email') returning event_id into event_id_value;
  end if;
  ...
  if p_operation='claim' then
   ...
   if p_operation='progress' then
    ...
    if state not in ('done','not_needed') or state is null or not exists(select 1 from jsonb_array_elements(job.body->'actions') a where a->>'id'=p_payload->>'action_id') then raise exception 'BUNCH_NOTE_PROGRESS_INVALID'; end if;
    ...
    update bunch_note_private.jobs set progress=progress||jsonb_build_object(p_payload->>'action_id',jsonb_build_object(
     'status',state,'reason',p_payload->>'reason','actor_id',actor.id,'at',now())) where id=job.id;
   else
    if exists(select 1 from jsonb_array_elements(job.body->'actions') a where not(job.progress ? (a->>'id'))) then raise exception 'BUNCH_NOTE_UNRESOLVED_ACTIONS'; end if;
  ...
  if p_operation<>'revise' then
   ...
   result:=jsonb_build_object('job',to_jsonb(job));
  end if;
...
-- Only the trusted PDF renderer calls this; preview bytes are immutable once prepared.
create function public.bunch_note_freeze_pdfs_v1(p_preview_id uuid,p_pdfs jsonb) returns jsonb
language plpgsql security definer set search_path='' as $$
declare p bunch_note_private.previews; f jsonb; total bigint:=0;
begin
 if not private.is_service_role_request() then raise exception 'BUNCH_NOTE_FORBIDDEN' using errcode='42501'; end if;
 select * into p from bunch_note_private.previews where id=p_preview_id for update;
 if not found or p.expires_at<=now() or p.published then raise exception 'BUNCH_NOTE_PREVIEW_EXPIRED'; end if;
 if p.pdfs is not null then return jsonb_build_object('pdfs',p.pdfs); end if;
...
create function public.bunch_note_delivery_lookup_v1(p_event_id uuid) returns jsonb
language plpgsql security definer set search_path='' as $$
...
begin
 ...
 return jsonb_build_object('event_id',e.event_id,'event_key',e.event_key,'event_type',e.event_type,'created_by',p.created_by,
 'reports',p.reports,'pdfs',p.pdfs,'recipients',p.recipients,'receipt',e.channel_results->'email',
 'delivery_status',coalesce(e.channel_results->'email'->>'status','pending'));
end $$;

create function public.bunch_note_delivery_record_v1(p_event_id uuid,p_lease_token uuid,p_status text,p_result jsonb) returns jsonb
language plpgsql security definer set search_path='' as $$
declare e public.ph_request_delivery_outbox; prior text; email jsonb;
begin
 if not private.is_service_role_request() then raise exception 'BUNCH_NOTE_FORBIDDEN' using errcode='42501'; end if;
 select * into e from public.ph_request_delivery_outbox where event_id=p_event_id and event_type='bunch_note_submission' for update;
 if not found or e.status<>'processing' or p_lease_token is null or e.lease_token is distinct from p_lease_token
  or e.lease_expires_at is null or e.lease_expires_at<=now() then raise exception 'BUNCH_NOTE_DELIVERY_LEASE_LOST'; end if;
 prior:=coalesce(e.channel_results->'email'->>'status','pending');
 if prior='sent' or (p_status='sending' and prior in ('sending','unknown')) then return jsonb_build_object('allow_send',false); end if;
 if p_status not in ('sending','sent','unknown','failed') or p_status is null or octet_length(p_result::text)>32000 then raise exception 'BUNCH_NOTE_DELIVERY_INVALID'; end if;
 if p_status='sent' and nullif(p_result->>'gmail_message_id','') is null then raise exception 'BUNCH_NOTE_RECEIPT_REQUIRED'; end if;
 if p_status='failed' and (prior in ('sending','unknown') or p_result->'safe_to_retry' is distinct from 'true') then raise exception 'BUNCH_NOTE_RECONCILIATION_REQUIRED'; end if;
 email:=coalesce(e.channel_results->'email','{}')||p_result||jsonb_build_object('status',p_status,'updated_at',now());
 ...
 update bunch_note_private.previews set delivery_status=p_status where event_id=p_event_id;
 return jsonb_build_object('allow_send',p_status='sending');
end $$;

create policy bunch_note_outbox_private on public.ph_request_delivery_outbox as restrictive for select to authenticated
 using(event_type<>'bunch_note_submission' or (select public.hl_order_can_read_outbox_v1()));
-- Keep legacy SECURITY DEFINER recovery endpoints away from Bunch Note deliveries.
do $$ declare name text; definition text; begin
 foreach name in array array['public.get_request_delivery_recovery_queue()','public.requeue_request_delivery(uuid)'] loop
  definition:=pg_get_functiondef(name::regprocedure);
  definition:=replace(definition,'''hl_order_submission'',''hl_order_cancellation''','''hl_order_submission'',''hl_order_cancellation'',''bunch_note_submission''');
  execute definition;
 end loop;
end $$;
revoke all on all functions in schema bunch_note_private from public,anon,authenticated,service_role;
revoke all on function public.bunch_note_command_v1(uuid,text,jsonb,uuid,bigint),public.bunch_note_freeze_pdfs_v1(uuid,jsonb),
 public.bunch_note_delivery_lookup_v1(uuid),public.bunch_note_delivery_record_v1(uuid,uuid,text,jsonb) from public,anon,authenticated;
grant execute on function public.bunch_note_command_v1(uuid,text,jsonb,uuid,bigint),public.bunch_note_freeze_pdfs_v1(uuid,jsonb),
 public.bunch_note_delivery_lookup_v1(uuid),public.bunch_note_delivery_record_v1(uuid,uuid,text,jsonb) to service_role;
-- Scoped metadata invalidation reuses the app's revision coordinator. Every
-- active user can observe these opaque counters; only the command API returns
-- authorized job contents, and ownership changes invalidate losing clients.
insert into app_sync_private.sources(key,modules,dylan_only) values
 ('bunch_note_private.jobs','{}',false),('bunch_note_private.previews','{}',false);
insert into public.app_dataset_revisions(key) values('bunch_note_private.jobs'),('bunch_note_private.previews');
do $$ declare t text; begin
 foreach t in array array['jobs','previews'] loop
  execute format('create trigger app_dataset_revision_inserted after insert on bunch_note_private.%I referencing new table as app_dataset_new_rows for each statement execute function app_sync_private.touch_source()',t);
  execute format('create trigger app_dataset_revision_updated after update on bunch_note_private.%I referencing new table as app_dataset_new_rows for each statement execute function app_sync_private.touch_source()',t);
  execute format('create trigger app_dataset_revision_deleted after delete on bunch_note_private.%I referencing old table as app_dataset_old_rows for each statement execute function app_sync_private.touch_source()',t);
 end loop;
end $$;
do $$ declare signature text; definition text; changed text; begin
 foreach signature in array array['app_sync_private.can_read_source(text)','app_sync_private.snapshot(text[])'] loop
  definition:=pg_get_functiondef(signature::regprocedure);
  changed:=replace(definition,'case when source.key like',
   'case when source.key in (''bunch_note_private.jobs'',''bunch_note_private.previews'') then true when source.key like');
  if changed=definition then raise exception 'BUNCH_NOTE_SYNC_CONTRACT_CHANGED'; end if;
  execute changed;
 end loop;
end $$;
commit;
