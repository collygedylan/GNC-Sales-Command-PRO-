-- Definition-only correction: do not repair business rows or replay notifications.
begin;
...
begin
  ...
  with latest_completion as (
    select distinct on (history.master_id)
      ...
      master.av_rule_last_cleared_at,
      master.av_rule_last_clear_reason,
    ...
    select completion.*,
      ...
      and (completion.av_rule_last_cleared_at is null
           or completion.av_rule_last_cleared_at < completion.completed_at) as has_repairable_mismatch
...
begin
  ...
  loop
    ...
    -- An equal or later shared reset supersedes the old completion evidence.
    if candidate.av_rule_last_cleared_at is not null
       and candidate.av_rule_last_cleared_at >= completed_at then
    ...
    if not coalesce(p_dry_run, true) then
      ...
      where master.unique_id = candidate.master_id
        and master.av_rule_last_cleared_at is not distinct from candidate.av_rule_last_cleared_at
        and master.av_rule_last_clear_reason is not distinct from candidate.av_rule_last_clear_reason
...
comment on function public.get_request_drive_evidence_health_snapshot_v1() is
  'Service-only health snapshot. Flags only non-empty completion evidence that can be repaired without superseding a shared reset or newer master-field update.';
comment on function public.repair_request_drive_evidence_v1(text[], boolean) is
  'Service-only bounded repair. Never clears a master field from empty history and uses optimistic reset/timestamp/value guards against concurrent or newer writes.';
