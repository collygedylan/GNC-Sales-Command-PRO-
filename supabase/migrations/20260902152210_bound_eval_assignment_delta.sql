...
-- Complete the bounded reconciliation repair by sharing one materialized set
-- of current assignment keys across both delta upsert and removal detection.
-- Only new, changed, or reactivated keys reach ON CONFLICT; unchanged keys do
-- not acquire row locks or rewrite timestamps.
...
begin
  ...
  with normalized_drive as materialized (
    ...
    left join public.ph_warehouse_assigned_items a
      on a.assignment_key = d.assignment_key
    where a.assignment_key is null
       or a.itemcode is distinct from d.itemcode_normalized
       or a.itemcode_normalized is distinct from d.itemcode_normalized
       or a.genusname is distinct from d.genusname
       or a.genusname_normalized is distinct from d.genusname_normalized
       or a.concat is distinct from (d.itemcode_normalized || d.genusname)
       or a.commonname is distinct from d.commonname
       or a.contsize is distinct from d.contsize
       or a.locationcode is distinct from d.locationcode
       or not a.present_in_drive
  ), written as (
  ...
  select
    (select count(*) filter (where was_inserted)::integer from written),
    (select count(*) filter (where not was_inserted)::integer from written),
    (select count(*)::integer from removed_rows)
