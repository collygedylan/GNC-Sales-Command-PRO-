begin;
-- Work records are independent of instruction revisions and imported inventory.
create table bunch_note_private.options (
 id uuid primary key default gen_random_uuid(), category text not null check(category in ('sequence','grading','placement','inventory','hauling')),
 label text not null check(length(btrim(label)) between 1 and 160), kind text not null check(kind in ('instruction','ta','move','hauling')),
 active boolean not null default true, revision bigint not null default 1, created_by uuid, updated_at timestamptz not null default now(),
 check(kind<>'hauling' or category='hauling')
);
create unique index bunch_note_option_label on bunch_note_private.options(category,lower(btrim(label))) where active;
create table bunch_note_private.worker_actions (
 id uuid primary key default gen_random_uuid(), job_id uuid not null references bunch_note_private.jobs(id),
 action jsonb not null, actor_id uuid not null, created_at timestamptz not null default now()
);
create index bunch_note_worker_action_job on bunch_note_private.worker_actions(job_id);
create table bunch_note_private.actuals (
 id uuid primary key default gen_random_uuid(), job_id uuid not null references bunch_note_private.jobs(id), action_id text not null,
 action_snapshot jsonb not null, source_snapshot jsonb not null, quantity numeric not null check(quantity>=0 and quantity<1000000000),
 destination text not null default '', explanation text not null default '', review_flags jsonb not null default '[]',
 replaces_id uuid unique references bunch_note_private.actuals(id), actor_id uuid not null, created_at timestamptz not null default now()
);
create index bunch_note_actual_job_action on bunch_note_private.actuals(job_id,action_id);
alter table bunch_note_private.previews add column report_kind text not null default 'instructions' check(report_kind in ('instructions','completed_work'));
alter table bunch_note_private.previews add column job_id uuid references bunch_note_private.jobs(id);
alter table bunch_note_private.previews add column job_revision bigint;
create unique index bunch_note_work_report_version on bunch_note_private.previews(job_id,job_revision) where published and report_kind='completed_work';
alter table bunch_note_private.options enable row level security;
alter table bunch_note_private.worker_actions enable row level security;
alter table bunch_note_private.actuals enable row level security;
revoke all on bunch_note_private.options,bunch_note_private.worker_actions,bunch_note_private.actuals from public,anon,authenticated,service_role;

insert into bunch_note_private.options(category,label,kind)
select category,label,case when label='TA / culls follow-up' then 'ta' when label='Move' then 'move'
 when category='hauling' and label<>'Wait for hauling before bunching' then 'hauling' else 'instruction' end
from (values
 ('sequence',array['Early protection','Haul-in space','Small-container crew','Drill shift','LD planting','Shovel shift','Rain-day preparation']),
 ('grading',array['Grade quantity or percentage','Dump quantity or percentage','Shear before bunching']),
 ('hauling',array['Grade shift','Row-run shift','Haul to destination','Wait for hauling before bunching']),
 ('placement',array['Center house','Outside house','Extra spacing','Combine groups','Water control','Won’t-fit stock','Overwinter outside','Countable rows','Field signs at both ends facing the road','Missing tags or signs','Variety mixes','Separate look-alikes','Wide aisles','Group symbols','Flag or ribbon guidance']),
 ('inventory',array['TA / culls follow-up','Move','Designated-location review','Pull-tag notes','Priority review','Season error review','Obsolete-location review'])
) t(category,labels) cross join lateral unnest(labels) label;

create function bunch_note_private.shift_eligible(r jsonb) returns boolean language sql immutable set search_path='' as $$
 select upper(coalesce(r->>'season','')) like '%Y%' or upper(coalesce(r->>'season','')) like '%U3%'
 or upper(coalesce(r->>'desigitem','')) like '%SHFT%'
$$;
create function bunch_note_private.number_value(v text) returns numeric language sql immutable set search_path='' as $$
 select case when btrim(v) ~ '^-?[0-9]+([.][0-9]+)?$' then btrim(v)::numeric else null end
$$;
create function bunch_note_private.action_kind(a jsonb) returns text language sql immutable set search_path='' as $$
 select coalesce(nullif(a->>'kind',''),case when coalesce(a->>'label',a->>'instructions','') ilike 'TA / culls%' then 'ta'
 when coalesce(a->>'label',a->>'instructions','')='Move' then 'move'
 when a->>'group'='hauling' and coalesce(a->>'label',a->>'instructions','') not ilike 'Wait%' then 'hauling' else 'instruction' end)
$$;
create function bunch_note_private.actions(j bunch_note_private.jobs) returns jsonb language sql stable set search_path='' as $$
 select coalesce((j).body->'actions','[]')||coalesce((select jsonb_agg(w.action order by w.created_at,w.id) from bunch_note_private.worker_actions w where w.job_id=(j).id),'[]')
$$;
create function bunch_note_private.actual_history(p_job uuid) returns jsonb language sql stable set search_path='' as $$
 select coalesce(jsonb_agg(to_jsonb(a)||jsonb_build_object('superseded',exists(select 1 from bunch_note_private.actuals newer where newer.replaces_id=a.id)) order by a.created_at,a.id),'[]')
 from bunch_note_private.actuals a where a.job_id=p_job
$$;
create function bunch_note_private.work_source(j bunch_note_private.jobs) returns jsonb language sql stable set search_path='' as $$
 select coalesce(jsonb_agg(r order by r->>'unique_id'),'[]') from (
  select distinct on(r->>'unique_id') r from (
   select r,0 priority from jsonb_array_elements((j).body->'source') r
   union all select r,1 from bunch_note_private.worker_actions w cross join lateral jsonb_array_elements(w.action->'source_snapshot') r where w.job_id=(j).id
  ) q order by r->>'unique_id',priority
 ) deduplicated
$$;
create function bunch_note_private.job_json(j bunch_note_private.jobs) returns jsonb language sql stable set search_path='' as $$
 select to_jsonb(j)||jsonb_build_object('body',jsonb_set((j).body,'{source}',bunch_note_private.work_source(j)),
 'worker_actions',coalesce((select jsonb_agg(w.action order by w.created_at,w.id) from bunch_note_private.worker_actions w where w.job_id=(j).id),'[]'),
 'actuals',bunch_note_private.actual_history((j).id))
$$;

-- Validate new instructions against the server catalog. Saved choices are snapshots;
-- renaming or retiring a choice never rewrites an existing instruction.
create function bunch_note_private.validate_action(a jsonb,source jsonb,prior_actions jsonb default '[]') returns jsonb
language plpgsql set search_path='' as $$
declare opt bunch_note_private.options; old jsonb; targets jsonb; kind text;
begin
 if coalesce(a->>'group','') not in ('sequence','grading','placement','identification','inventory','hauling') or nullif(a->>'id','') is null or length(coalesce(a->>'instructions','')) not between 1 and 4000
 or coalesce(a->>'scope','') not in ('rows','location') then raise exception 'BUNCH_NOTE_ACTION_INVALID'; end if;
 select x into old from jsonb_array_elements(prior_actions) x where x->>'id'=a->>'id';
 if old is not null then
  if (a->>'option_id') is distinct from (old->>'option_id') or (a->>'kind') is distinct from (old->>'kind')
   or (a->>'group') is distinct from (old->>'group') or (a->>'label') is distinct from (old->>'label') then raise exception 'BUNCH_NOTE_ACTION_ID_REUSED'; end if;
 elsif nullif(a->>'option_id','') is not null then
  select * into opt from bunch_note_private.options where id=(a->>'option_id')::uuid and active;
  if not found or a->>'group' is distinct from opt.category or a->>'kind' is distinct from opt.kind or a->>'label' is distinct from opt.label then raise exception 'BUNCH_NOTE_OPTION_CHANGED'; end if;
 else
  -- Compatible older clients can still save ordinary instructions. Quantity-bearing
  -- built-in actions get their trusted kind from the catalog, never a client flag.
  select * into opt from bunch_note_private.options where label=coalesce(a->>'label',a->>'instructions') and active;
  if nullif(a->>'kind','') is not null then raise exception 'BUNCH_NOTE_OPTION_REQUIRED'; end if;
  if found then a:=a||jsonb_build_object('kind',opt.kind); end if;
 end if;
 kind:=bunch_note_private.action_kind(a);
 if kind not in ('instruction','ta','move','hauling') then raise exception 'BUNCH_NOTE_ACTION_INVALID'; end if;
 if a->>'scope'='rows' and (jsonb_typeof(a->'row_ids') is distinct from 'array' or jsonb_array_length(a->'row_ids')=0) then raise exception 'BUNCH_NOTE_ACTION_ROWS_INVALID'; end if;
 select coalesce(jsonb_agg(r),'[]') into targets from jsonb_array_elements(source) r where a->>'scope'='location' or a->'row_ids' ? (r->>'unique_id');
 if jsonb_array_length(targets)=0 or (a->>'scope'='rows' and exists(select 1 from jsonb_array_elements_text(a->'row_ids') id where not exists(select 1 from jsonb_array_elements(targets) r where r->>'unique_id'=id))) then raise exception 'BUNCH_NOTE_ACTION_ROWS_INVALID'; end if;
 if (a->>'group'='hauling' or kind='hauling') and exists(select 1 from jsonb_array_elements(targets) r where not bunch_note_private.shift_eligible(r)) then raise exception 'BUNCH_NOTE_SHIFT_ROWS_INVALID'; end if;
 return a;
end $$;

create function bunch_note_private.record_actual(j bunch_note_private.jobs,p jsonb,actor uuid) returns void
language plpgsql set search_path='' as $$
declare a jsonb; r jsonb; prior bunch_note_private.actuals; amount numeric; total numeric; planned numeric; stock numeric; flags jsonb:='[]'; kind text; dest text; reason text;
begin
 if nullif(p->>'replaces_id','') is not null then
  select * into prior from bunch_note_private.actuals where id=(p->>'replaces_id')::uuid and job_id=j.id;
  if not found or exists(select 1 from bunch_note_private.actuals where replaces_id=prior.id) then raise exception 'BUNCH_NOTE_AMENDMENT_CONFLICT'; end if;
  a:=prior.action_snapshot; r:=prior.source_snapshot;
  if p->>'action_id' is distinct from prior.action_id or p->>'source_id' is distinct from r->>'unique_id' then raise exception 'BUNCH_NOTE_AMENDMENT_INVALID'; end if;
 else
  select x into a from jsonb_array_elements(bunch_note_private.actions(j)) x where x->>'id'=p->>'action_id';
  select x into r from jsonb_array_elements(bunch_note_private.work_source(j)) x where x->>'unique_id'=p->>'source_id';
 end if;
 if a is null or r is null or (a->>'scope'='rows' and not(a->'row_ids' ? (r->>'unique_id'))) then raise exception 'BUNCH_NOTE_ACTION_ROWS_INVALID'; end if;
 kind:=bunch_note_private.action_kind(a);
 if kind not in ('ta','move','hauling') then raise exception 'BUNCH_NOTE_ACTUAL_TYPE_REQUIRED'; end if;
 if nullif(btrim(r->>'contsize'),'') is null or nullif(btrim(r->>'lotcode'),'') is null then raise exception 'BUNCH_NOTE_LOT_SIZE_REQUIRED'; end if;
 if (a->>'group'='hauling' or kind='hauling') and not bunch_note_private.shift_eligible(r) then raise exception 'BUNCH_NOTE_SHIFT_ROWS_INVALID'; end if;
 if coalesce(p->>'quantity','') !~ '^[0-9]+$' then raise exception 'BUNCH_NOTE_QUANTITY_INVALID'; end if;
 amount:=(p->>'quantity')::numeric; dest:=btrim(coalesce(p->>'destination','')); reason:=btrim(coalesce(p->>'explanation',''));
 if amount>=1000000000 or (amount=0 and prior.id is null) then raise exception 'BUNCH_NOTE_QUANTITY_INVALID'; end if;
 if kind in ('move','hauling') and (length(dest) not between 1 and 200) then raise exception 'BUNCH_NOTE_DESTINATION_REQUIRED'; end if;
 if length(reason)>4000 then raise exception 'BUNCH_NOTE_REASON_INVALID'; end if;
 if prior.id is not null and reason='' then raise exception 'BUNCH_NOTE_REASON_REQUIRED'; end if;
 select coalesce(sum(x.quantity),0)+amount into total from bunch_note_private.actuals x
 where x.job_id=j.id and x.source_snapshot->>'unique_id'=r->>'unique_id' and (prior.id is null or x.id<>prior.id)
 and not exists(select 1 from bunch_note_private.actuals newer where newer.replaces_id=x.id);
 stock:=bunch_note_private.number_value(r->>'stock');
 if stock is not null and total>stock then flags:=flags||'"exceeds_saved_stock"'::jsonb; end if;
 if stock is null then flags:=flags||'"saved_stock_unknown"'::jsonb; end if;
 planned:=bunch_note_private.number_value(a->>'quantity');
 if planned is null and bunch_note_private.number_value(a->>'percentage') is not null then
  select case when count(*)=count(bunch_note_private.number_value(x->>'stock')) then sum(bunch_note_private.number_value(x->>'stock'))*bunch_note_private.number_value(a->>'percentage')/100 end into planned
  from jsonb_array_elements(j.body->'source') x where a->>'scope'='location' or a->'row_ids' ? (x->>'unique_id');
 end if;
 select coalesce(sum(x.quantity),0)+amount into total from bunch_note_private.actuals x where x.job_id=j.id and x.action_id=a->>'id'
 and (prior.id is null or x.id<>prior.id) and not exists(select 1 from bunch_note_private.actuals newer where newer.replaces_id=x.id);
 if planned is not null and total>planned then flags:=flags||'"exceeds_planned_quantity"'::jsonb; end if;
 if j.status='complete' and planned is not null and total<>planned then flags:=flags||'"differs_from_planned_quantity"'::jsonb; end if;
 if jsonb_array_length(flags)>0 and reason='' then raise exception 'BUNCH_NOTE_VARIANCE_REASON_REQUIRED'; end if;
 insert into bunch_note_private.actuals(job_id,action_id,action_snapshot,source_snapshot,quantity,destination,explanation,review_flags,replaces_id,actor_id)
 values(j.id,a->>'id',a,r,amount,dest,reason,flags,prior.id,actor);
 -- An amendment or another partial entry requires the open action to be checked again.
 if j.status='open' then update bunch_note_private.jobs set progress=progress-(a->>'id') where id=j.id; end if;
end $$;

create function bunch_note_private.check_done(j bunch_note_private.jobs,a jsonb,reason text) returns jsonb
language plpgsql set search_path='' as $$
declare total numeric; planned numeric; flags jsonb:='[]';
begin
 if bunch_note_private.action_kind(a) not in ('ta','move','hauling') then return flags; end if;
 select sum(x.quantity) into total from bunch_note_private.actuals x where x.job_id=j.id and x.action_id=a->>'id'
 and not exists(select 1 from bunch_note_private.actuals newer where newer.replaces_id=x.id);
 if total is null or total<=0 then raise exception 'BUNCH_NOTE_ACTUAL_REQUIRED'; end if;
 planned:=bunch_note_private.number_value(a->>'quantity');
 if planned is null and bunch_note_private.number_value(a->>'percentage') is not null then
  select case when count(*)=count(bunch_note_private.number_value(x->>'stock')) then sum(bunch_note_private.number_value(x->>'stock'))*bunch_note_private.number_value(a->>'percentage')/100 end into planned
  from jsonb_array_elements(j.body->'source') x where a->>'scope'='location' or a->'row_ids' ? (x->>'unique_id');
 ...
 if planned is not null and total<>planned then
  if nullif(btrim(reason),'') is null then raise exception 'BUNCH_NOTE_VARIANCE_REASON_REQUIRED'; end if;
  flags:=jsonb_build_array('differs_from_planned_quantity');
 end if;
 return flags;
end $$;

-- Corrected inventory projection
create or replace function bunch_note_private.inventory(p_block text,p_location text default null) returns jsonb
language sql stable security definer set search_path='' as $$
 select coalesce(jsonb_agg(jsonb_build_object(
  'unique_id',m.unique_id,'blockalpha',m.blockalpha,'locationcode',m.locationcode,
  'itemcode',m.itemcode,'commonname',m.commonname,'contsize',m.contsize,'lotcode',m.lotcode,
  'season',to_jsonb(m)->>'season','desigitem',to_jsonb(m)->>'desigitem',
  'stock',m.ptronhand,'review',m.ptrreviewed,'available',m.ptravailable,
  'flags',concat_ws(' / ',nullif(to_jsonb(m)->>'qualitycode',''),nullif(to_jsonb(m)->>'field_tag_color',''),nullif(to_jsonb(m)->>'fieldtagcolor','')),
  'location_notes',coalesce(to_jsonb(m)->>'locationnote',to_jsonb(m)->>'locnote',''),
  'warehouse',concat_ws(' / ',nullif(to_jsonb(m)->>'warehouseid',''),nullif(to_jsonb(m)->>'warehousename',''),nullif(to_jsonb(m)->>'warehousei','')),
  'hold',concat_ws(' / ',nullif(to_jsonb(m)->>'lochold',''),nullif(to_jsonb(m)->>'holdstopcode',''),nullif(to_jsonb(m)->>'holdstopreason',''))
 ) order by m.unique_id),'[]') from public.ph_master_inventory m
 where upper(btrim(m.blockalpha))=p_block and nullif(btrim(m.locationcode),'') is not null
 and (p_location is null or upper(btrim(m.locationcode))=p_location)
$$;
...
begin
 ...
 if p_operation in ('option_add','option_edit') then
  ...
  else perform bunch_note_private.record_actual(job,p_payload,actor.id);
  end if;
  ...
  for entry in select value from jsonb_array_elements(body->'locations') loop
   ...
   if jsonb_typeof(entry->'actions') is distinct from 'array' or jsonb_array_length(entry->'actions') not between 1 and 200
    or length(coalesce(entry->>'instructions',''))>8000 or length(coalesce(entry->>'prerequisites',''))>8000
    or length(coalesce(entry->>'purposes','')) not between 1 and 500 then raise exception 'BUNCH_NOTE_INSTRUCTIONS_REQUIRED'; end if;
  ...
  body:=jsonb_build_object('snapshot_version',2,'block',upper(btrim(body->>'block')),'locations',reports,'recipient_ids',coalesce(body->'recipient_ids','[]'));
...
-- All new implementation helpers stay private. Existing service-only entrypoints
-- retain their grants, source-scoped refreshes and delivery reconciliation rules.
revoke all on all functions in schema bunch_note_private from public,anon,authenticated,service_role;
