...
select like(pg_get_functiondef('private.save_drive_evidence_core_v2(text,text,text,text,text,jsonb,jsonb,boolean,text,text,boolean,text)'::regprocedure), '%pg_try_advisory_xact_lock%', 'save path uses try-locks');
select unlike(pg_get_functiondef('private.save_drive_evidence_core_v2(text,text,text,text,text,jsonb,jsonb,boolean,text,text,boolean,text)'::regprocedure), '%FOR UPDATE%', 'save path never queues on a row lock');
select like(pg_get_functiondef('private.save_drive_evidence_core_v2(text,text,text,text,text,jsonb,jsonb,boolean,text,text,boolean,text)'::regprocedure), '%last_updated IS NOT DISTINCT FROM v_row.last_updated%', 'save path uses a conditional atomic update');
