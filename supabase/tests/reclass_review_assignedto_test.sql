...
update public.profiles set display_name=repeat('Recipient ',1000) where username='review_extra';
select throws_ok($q$select public.create_eval_work_multi_v1((select payload from review_packets where name='create'))$q$,'22023','REVIEW_RECIPIENT_INVALID','oversized frozen names rejected with sanitized guidance');
select is((select count(*)::integer from public.ph_eval_work where create_token='review-assignedto-test-created-0001'),0,'oversized recipients do not create work');
update public.profiles set display_name='Review review_extra' where username='review_extra';
