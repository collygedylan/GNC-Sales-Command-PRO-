...
function renderRequestGalleryWebApp_(params) {
  ...
  const html = [
    ...
    '.bottom{position:fixed;left:0;right:0;bottom:0;z-index:5;padding:10px 14px calc(12px + env(safe-area-inset-bottom));background:linear-gradient(0deg,rgba(0,0,0,.9),rgba(0,0,0,0));}',
    '.caption{display:none;}',
    '.controls{max-width:980px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px;}',
    ...
    slides.length ? [
      ...
      '<footer class="bottom">',
      '<div class="caption">',
      '<h2 id="slideTitle"></h2><p id="slideSubtitle"></p>',
      '</div>',
      '<div class="controls">',
      '<button type="button" class="control secondary" id="bottomPrevBtn">Prev</button>',
      '<button type="button" class="control secondary" id="openBtn">Open Link</button>',
      '<button type="button" class="control secondary" id="copyBtn">Copy Link</button>',
    ...
    'function render(){var slide=slides[index]||{};var count=(index+1)+" / "+slides.length;image.src=slide.url||"";image.alt=slide.title||"Request photo";title.textContent=slide.title||"Request photo";subtitle.textContent=slide.subtitle||"";topTitle.textContent=String(slide.title||"Request photo").toUpperCase();topCounter.textContent=count;}',
    'function go(delta){index=(index+delta+slides.length)%slides.length;render();}',
    'prev.addEventListener("click",function(){go(-1);});',
    'next.addEventListener("click",function(){go(1);});',
    'bottomPrev.addEventListener("click",function(){go(-1);});',
    'bottomNext.addEventListener("click",function(){go(1);});',
    'openBtn.addEventListener("click",function(){var slide=slides[index]||{};if(slide.url)window.open(slide.url,"_blank","noopener");});',
    'copyBtn.addEventListener("click",function(){var slide=slides[index]||{};if(!slide.url)return;if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(slide.url).then(function(){copyBtn.textContent="Copied";setTimeout(function(){copyBtn.textContent="Copy Link";},1200);});}else{window.prompt("Copy photo link",slide.url);}});',
