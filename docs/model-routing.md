...
The project `.codex/config.toml` selects **GPT-5.6 Sol / medium** for new sessions and **GPT-5.6 Terra / medium** for delegated work by default. The owner chooses explicit model and effort values for each useful delegated task:

| Work | Model and effort |
| --- | --- |
| Clear, bounded text edits, mechanical transformations, or short summaries | `gpt-5.6-luna`, low |
| Targeted exploration, log triage, routine isolated implementation | `gpt-5.6-terra`, medium |
| New views, related client/server changes, database migrations, independent correctness review | `gpt-5.6-sol`, high |
...
Start at the appropriate tier; do not first attempt a risky migration with a model chosen for mechanical edits. Increase effort or use Astra when concrete complexity warrants it. Max/Ultra are not routine defaults. Explain an escalation in one sentence; no repeated permission request is needed for model selection within the authorized task. If a tier is unavailable, report that limitation rather than silently using a more expensive tier.
...
- For useful delegation, give a concise task, file ownership, relevant context, and expected output. Do not fork the entire task history into a differently configured worker.
