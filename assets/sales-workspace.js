(function (root) {
 ...
 const field = (row, key) => row?.[key] ?? row?.[key.toUpperCase()] ?? '';
 ...
 function isView(view) { return views.includes(view); }
 ...
 async function api(action,operation,payload={},revision=null,mutate=false) {
  ensureAccount(); const owner=state, key=JSON.stringify([action,operation,payload,revision]);
  const commandId=mutate?(state.commands.get(key)||uuid()):null;
  if(commandId)state.commands.set(key,commandId);
  const response=await postAppFunctionJson(APP_API_FUNCTION_URL,{action,operation,payload,...(revision===null?{}:{expectedRevision:revision}),...(commandId?{commandId}:{})},{timeoutMs:45000,label:action==='request_history'?'Request History':'Credit',idempotencyKey:commandId||undefined});
  if(owner!==state||owner.account!==account())throw new Error('The signed-in account changed.');
  if(!response?.ok)throw new Error(response?.message||response?.error||response?.code||'Unable to save. Your entries are retained.');
  if(commandId)state.commands.delete(key);
 ...
 function compose(){
  ...
  park();const selected=[...state.selection.values()];
  if(state.draft)state.localDrafts.set(state.draft.customerKey,state.draft);state.draft=state.localDrafts.get(state.customerKey)||{id:uuid(),revision:0,customerKey:state.customerKey,lines:[]};state.localDrafts.set(state.customerKey,state.draft);
  if(!state.draft.pendingSubmit)for(const row of selected)if(!state.draft.lines.some(line=>line.sourceId===row.id))state.draft.lines.push({id:uuid(),sourceId:row.id,quantity:'',explanation:'',attachmentIds:[],source:row});
  state.mode='compose';render();return false;
 }
 function editLine(id,key,value){const line=state.draft?.lines.find(l=>l.id===id);if(!state.draft?.pendingSubmit&&line&&['quantity','explanation'].includes(key))line[key]=value;}
 function removeLine(id){if(state.busy||state.draft?.pendingSubmit)return;state.draft.lines=state.draft.lines.filter(l=>l.id!==id);state.files.delete(id);render();}
 function addFiles(id,input){if(state.busy||state.draft?.pendingSubmit)return;const files=Array.from(input.files||[]);const existing=state.files.get(id)||[];for(const file of files){if(!file.type.startsWith('image/')){state.error='Choose image files.';continue;}existing.push({id:uuid(),file});}state.files.set(id,existing);input.value='';render();}
 function removeFile(id,fileId){if(state.busy||state.draft?.pendingSubmit)return;state.files.set(id,(state.files.get(id)||[]).filter(f=>f.id!==fileId));render();}
 const fileBase64=file=>new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',')[1]);reader.onerror=()=>reject(new Error('The photo could not be read. Choose it again.'));reader.readAsDataURL(file);});
 async function uploadFiles(){const owner=state;for(const line of state.draft.lines){for(const entry of [...(state.files.get(line.id)||[])]){if(!entry.prepared){const image=typeof optimizePhotoBlobForUpload==='function'?await optimizePhotoBlobForUpload(entry.file,entry.file.name):{blob:entry.file,fileName:entry.file.name};if(image.blob.size>8*1024*1024)throw new Error('Each photo must be 8 MB or smaller. Your photos are retained.');entry.prepared={sourceId:line.sourceId,mime:image.blob.type,base64:await fileBase64(image.blob),filename:image.fileName||entry.file.name};}if(owner!==state||owner.account!==account())throw new Error('The signed-in account changed.');const result=await api('sales_credit','attachment_upload',entry.prepared,null,true);line.attachmentIds=[...new Set([...line.attachmentIds,result.attachmentId])];state.files.set(line.id,(state.files.get(line.id)||[]).filter(f=>f.id!==entry.id));}}}
 function draftPayload(){const d=state.draft;return{id:d.id,customerKey:d.customerKey,lines:d.lines.map(({id,sourceId,quantity,explanation,attachmentIds})=>({id,sourceId,quantity:quantity===''?null:Number(quantity),explanation,attachmentIds}))};}
 async function saveDraft(){if(state.draft.pendingSubmit)throw new Error('The last submission may have succeeded. Retry submission to confirm its result before editing.');await uploadFiles();const result=await api('sales_credit','save_draft',draftPayload(),state.draft.revision,true);const saved=result.draft||result;state.draft.revision=saved.revision;return saved;}
 function save(){return run(async()=>{await saveDraft();showToast('Draft saved','Your credit entries have been saved.');});}
 function submit(){return run(async()=>{
  if(!state.draft.lines.length)throw new Error('Select at least one source row.');
  for(const line of state.draft.lines)if(!(Number(line.quantity)>0)||!Number.isFinite(Number(line.quantity))||!String(line.explanation).trim())throw new Error('Every line needs a positive affected quantity and a written explanation.');
 ...
 function decision(lineId,status){const reason=document.getElementById('credit-reason-'+lineId)?.value.trim()||'';return run(async()=>{const line=(state.detail.lines||[]).find(l=>l.id===lineId);if(status==='denied'&&!reason)throw new Error('Enter a reason for denying this line.');await api('sales_credit','review_line',{lineId,decision:status,reason},line.revision,true);const id=state.detail.submission?.id||line.submission_id;state.detail=await api('sales_credit','detail',{id});});}
 function authorize(index){const source=state.rows[index];const reason=document.getElementById('repeat-reason-'+source.id)?.value.trim();const requesterId=document.getElementById('repeat-requester-'+source.id)?.value;return run(async()=>{if(!reason)throw new Error('Explain why an additional claim is authorized.');await api('sales_credit','authorize_repeat',{sourceId:source.id,requesterId,reason},source.revision??null,true);await load();});}
 function resolveSource(index){const source=state.rows[index];const canonicalSourceId=document.getElementById('source-match-'+source.id)?.value;const reason=document.getElementById('source-reason-'+source.id)?.value.trim();return run(async()=>{if(!reason)throw new Error('Explain how you identified this shipment.');await api('sales_credit','resolve_source',{sourceId:source.id,...(canonicalSourceId?{canonicalSourceId}:{}),reason},source.revision,true);await load();});}
 ...
 function reviewHtml(){const data=state.detail||{};return `<h2>${esc(customer(data.lines?.[0]||data.submission))}</h2>${(data.lines||[]).map(line=>`<article class="sw-card"><h3>${esc(label(line))}</h3>${describe(line)}<p><strong>${esc(line.status)}</strong> · Affected quantity ${esc(line.quantity??line.credit_qty)}</p><p class="sw-pre">${esc(line.explanation||line.credit_reason||line.credit_note)}</p><div class="sw-actions">${photos(line.attachmentIds||line.attachment_ids)}</div>${line.reviewed_at?`<p>${esc(line.reviewed_by_display||line.reviewed_by_username)} · ${esc(line.reviewed_at)}</p>`:''}${line.review_note?`<p>Decision: ${esc(line.review_note)}</p>`:''}${line.canAmend?`<details data-sw-disclosure="amend-${esc(line.id)}"><summary>Correct this line</summary><label>Affected quantity<input id="amend-quantity-${esc(line.id)}" type="number" min="0" step="any" value="${esc(line.quantity??line.credit_qty)}"></label><label>Explanation<textarea id="amend-explanation-${esc(line.id)}">${esc(line.explanation||line.credit_reason||line.credit_note)}</textarea></label><label>Reason for correction<textarea id="amend-reason-${esc(line.id)}"></textarea></label>${button('Save audited correction',`SalesWorkspace.amend(${arg(line.id)})`)}</details>`:''}${(line.reviewHistory||[]).length?`<details data-sw-disclosure="history-${esc(line.id)}"><summary>Review history</summary>${line.reviewHistory.map(e=>`<p>${esc(e.created_at)} · ${esc(e.operation||e.event_type||e.action)} · ${esc(e.reason||'')}</p>`).join('')}</details>`:''}${data.canReview&&line.status==='pending'?`<label>Review reason (required for denial)<textarea id="credit-reason-${esc(line.id)}" rows="2"></textarea></label><div class="sw-actions">${button('Approve line',`SalesWorkspace.decision(${arg(line.id)},'approved')`)}${button('Deny line',`SalesWorkspace.decision(${arg(line.id)},'denied')`)}</div>`:''}</article>`).join('')}`;}
 function render(){
  if(typeof document==='undefined')return;ensureAccount();const container=document.getElementById(state.view+'-content');if(!container)return;
  const title={'request-history':'Request History','sales-credit':'Credit','credit-request':'Credit Request'}[state.view];
  let html=`<header><h1>${esc(title)}</h1></header>${state.error?`<div class="sw-error" role="alert">${esc(state.error)}<p>Your entries are retained. Review the message before retrying.</p></div>`:''}${state.busy?'<p role="status">Saving or loading…</p>':''}`;
  if(state.mode==='compose')html+=composeHtml();
  else if(state.mode==='review')html+=reviewHtml();
   ...
   html+=`<div class="sw-grid">${state.rows.map((row,i)=>state.mode==='folders'||state.view==='sales-credit'&&!state.customerKey?folderCard(row,i):state.view==='request-history'?historyCard(row,i):state.view==='credit-request'?`<button type="button" class="sw-card sw-open" onclick="SalesWorkspace.review(${i})"><strong>${esc(customer(row))}</strong><span>${esc(row.submitted_at||row.created_at)}</span><span>${esc(row.line_count??'')} matching lines</span></button>`:sourceCard(row,i)).join('')||(!state.busy?'<p>No matching records.</p>':'')}</div>`;
   if(state.nextCursor)html+=button('Load more','SalesWorkspace.more()');
   if(state.view==='sales-credit'&&state.customerKey)html+=`<div class="sw-primary">${button(`Prepare credit (${state.selection.size} rows)`,'SalesWorkspace.compose()',!state.selection.size)}</div>`;
  }
  const focus=document.activeElement, editing=focus&&container.contains(focus)&&focus.tagName==='INPUT'&&focus.type==='search',start=editing?focus.selectionStart:null;
  container.querySelectorAll('input[id],textarea[id],select[id]').forEach(el=>state.formValues.set(el.id,el.value));
  container.querySelectorAll('details[data-sw-disclosure]').forEach(el=>state.disclosures.set(el.dataset.swDisclosure,el.open));
  container.innerHTML=html;container.classList.add('sales-workspace');
  container.querySelectorAll('details[data-sw-disclosure]').forEach(el=>{el.open=!!state.disclosures.get(el.dataset.swDisclosure);});
