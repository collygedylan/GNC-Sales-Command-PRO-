(function (root) {
 ...
 function editor() {
  ...
  let html = `<h2>Bunch Note</h2>${viewTabs()}<p class="bn-muted">Choose a block, then a location to prepare its work.</p>`;
  ...
  if (!state.location) {
   ...
   const groups=locationGroups(names), shown=state.base ? groups.get(state.base)||[] : [...groups.keys()];
   html += `<h3>${state.base ? 'Full location / bay' : 'Location Code'}</h3><div class="bn-drill-grid">${shown.map(name=>drillCard(name,state.base ? `${(locations.get(name)||[]).length} plant rows` : `${groups.get(name).length} full locations`,state.base ? `BunchNote.openLocation(${arg(name)})` : `BunchNote.openBase(${arg(name)})`,'location')).join('')}</div>`;
 ...
 function queue() {
  ...
  let html = '<h2>Bunch Notes</h2>'+viewTabs()+'<p>Claim available location work. Claimed work is visible to its owner and Dylan.</p>';
